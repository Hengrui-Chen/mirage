...
class PersistentKernel:
    ...
    def linear_with_residual_layer(
        ...
        if scale is not None:
            # tb_graph.new_input(scale, (0, -1, -1,), 1, True)
            # the above may not be feasible at the moment because it would lead to error of /mirage/src/threadblock/input_loader.cc:101: mirage::threadblock::TBInputOp::TBInputOp(mirage::threadblock::Graph*, const mirage::kernel::DTensor&, int3, int, mirage::layout::SmemLayout, bool): Assertion `tensor.dim[dim_idx] % dim_div == 0' failed.
            # because 0 means we partition the first dimension, which is not divisible by 64 (set in grid_dim). 
            # you can refer to the code mpk.linear_with_residual_layer(
                #     input=attn_out,
                #     weight=w,
                #     residual=x,
