...
def get_fp8_tensors() -> Dict[int, Dict[str, torch.Tensor]]:
    files = [
        "/home/hengruic/models/Qwen3-8B-FP8/model-00001-of-00002.safetensors",
        "/home/hengruic/models/Qwen3-8B-FP8/model-00002-of-00002.safetensors",
    ]
    device = "cpu"  
    # 存储结果：layer_id -> {"weight_fp8": Tensor, "weight_scale_inv": Tensor}
    layer_weights = {}
    for path in files:
        with safe_open(path, framework="pt", device=device) as f:
            for key in f.keys():
                if "self_attn.o_proj" in key and ("weight" in key or "weight_scale_inv" in key):
                    # 从参数名提取层号
                    # 假设参数名形如: "model.layers.0.self_attn.o_proj.weight_fp8"
                    parts = key.split(".")
                    try:
                        layer_id = int(parts[2])  # 第3个字段是层号
                    except ValueError:
                        continue
...
if __name__ == "__main__":
    ...
    with torch.device("cuda"):
        if args.quant_fp8:
            ...
            # model_fp8 = build_all_layers_modulelist_from_files(files, device=torch.device("cuda"))
    ...
    if args.use_mirage:
        ...
        for i, layer in enumerate(model.model.layers):
            ...
            if args.quant_fp8:
                ...
                w = mpk.attach_input(
                    torch_tensor=w_tensor, name=f"layer_{i}_o_proj"
                )
